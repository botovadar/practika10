...
void fileprices(raspisanie* sched, const char* filename) {
	ifstream fin(filename);
	if (fin.is_open()) {
		int i = 0;
		while (!fin.eof() && i < SIZE) {
			fin >> sched[i].price;
			cout << sched[i].price << endl;
			i++;
		}
	}
	fin.close();
...
void savetobin(const char* filename, raspisanie* data, int size) {
	ofstream out(filename, ios::binary | ios::out);
	if (!out) {
		cout << "Error" << endl;
		return;
	}
	out.write((char*)data, sizeof(raspisanie) * size);
	out.close();
...
int main() {
	...
	fileprices(sched, "myfile.txt");
	cout << "Bin file writing:" << endl;
	savetobin("flights_data.bin", sched, SIZE);
